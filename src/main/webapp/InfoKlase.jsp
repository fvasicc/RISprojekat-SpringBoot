...
<body>
	...
	<div class="main">
		<c:if test="${not empty klase}">
			<table id="prikazLetova" border="1">
				<tr><th></th><th>Dozvoljeno kofera</th><th>Dozvoljena tezina prtljaga</th></tr>
				<c:forEach items="${klase}" var="k">
					<tr>
						<td>${k.naziv}</td>
						<td style="text-align: center;">${k.dozvoljenoKofera}</td>
						<td style="text-align: center;">${k.dozvoljenaTezinaPrtljaga}</td>
					</tr>
				</c:forEach>
			</table>
		</c:if>
